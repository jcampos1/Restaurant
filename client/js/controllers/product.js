...
angular
  .module('app')
  .controller('ProductController', ['$scope', 'Product', 'cm01', 'ms01', '$uibModal', '$location', '$log', function($scope, Product, cm01, ms01,
      $uibModal, $location, $log) {
        ...
        $scope.$watch(function() { return cm01.getEvnt06() }, function() {
          if( cm01.isValid(cm01.getEvnt06()) ){
            ...
            cm01.getData03().$save().then(function(instance){
              $scope.productFind();
              ms01.msgSuccess();
              cm01.setData03(null);
              cm01.setEvnt06(null);
  ...
  ['$scope', 'Product', 'Category', 'cm01', 'ms01', '$uibModalInstance', '$state', '$location', '$log', function($scope, Product, Category, cm01, ms01, $uibModalInstance,
    $state, $location, $log) {
      ...
      $scope.edit = function( form ) {
        if( form.$valid ) {
          $log.info("DATOS A ENVIAR AL SERVIDOR");
          $log.info("Category id");
          $log.info(beforeCat.id);
          $log.info("Foreign key for products");
          $log.info($scope.product.fk);
          Category.products.updateById(
            {id: beforeCat.id},
            {fk: $scope.product.fk}).$promise
          .then(function(product) {
          ...
          /*$scope.product.$save().then(function(instance){
            ms01.msgSuccess();
            cm01.setEvnt05("emit");
            cm01.setData03(null);
            $scope.cancel();
